<template>
  <div class="article">
    <br><br>
    <!-- <v-text-field label="Title" :readonly="true" v-model="title" style="text-align: center;"></v-text-field> -->
    <h1>{{title}}</h1>
...
export default {
  created(){
    this.config.data = JSON.parse(this.editor_text);
    ...
    save(){
      ...
      this.editor.save().then((data)=>{
        ...
        if(this.post_id == -1){
          ...
          fetch("http://localhost:8000/post/"+this.post_id.toSting()+"/", requestOptions)
          .then(response => response.text())
